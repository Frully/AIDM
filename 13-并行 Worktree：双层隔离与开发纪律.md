...
**并行多线程开发依赖于静态代码隔离与动态环境装配的双层物理防线，并需配合严格的生命周期清理纪律以防止资源冲突。**
...
在 gstack 体系中，并行分支的开发由一套内置的 `worktree` 管理脚本来支撑。

当决定对某个原子任务进行开发时，我们无需手动执行底层的 Git 指令，而是通过 gstack 派生出一个独立的开发沙箱：

```bash
# 为原子任务创建一个全新的开发目录与分支
$ gstack sandbox create --id RELAY_REFUND_01 --branch feat/refund-decision
```

这一命令在底层会自动执行 `git worktree add`，在同级目录生成专属的物理文件夹，并映射相应的跟踪分支。工具本身为可替换的实现层，其要旨在于将静态代码大仓和每个 Agent 会话的工作路径进行刚性互锁，配合生命周期钩子，确保开发过程的代码物理安全。
