...
**项目应当确立通用的 `AGENTS.md` 文件作为全局单一事实源，集中定义系统硬约束，并通过软链接实现跨 Agent 引擎的规则加载兼容。**
...
宪法应当保持短小精炼。它应当是一张快速匹配的拦截网，不提供长篇大论的学习教程。信息密度过低会导致模型在长上下文检索中发生关键红线遗忘。
...
重点在于理清这些原语在整体 Harness 回路中所扮演的角色。配置 Git Hook 的动机是为代码提交的物理关口强制触发 Lint 与安全扫描门禁，而不只是为了追求工具链的完整度。

## gstack 参考实现

在 gstack 体系中，上述工程原语内置且开箱即用。我们用它演示如何串联静态宪法与动态卡口，此工具是可替换的实现层：

* **Skills 命令扩展**：gstack 将生命周期的各核心卡口固化为内置的扩展指令（如 `/plan-review` 调起方案评审，`/ship` 自动校验流水线后进行灰度部署），Agent 可在命令行中无缝触发这些定义好的标准化工作流。
* **SessionStart Hook 挂载**：利用 Hook 机制，在 Agent 启动新的工作对话时，自动为当前分支挂接相对独立的 `worktree` 隔离环境，并将 `AGENTS.md` 注入其系统上下文。
